...
# ------------------------------------------------------------
def segment_graph(num_vertices, num_edges, edges, c, break_at_egde_i=None):
    ...
    for i in range(num_edges):
        ...
        b = u.find(pedge[1])
        if a != b:
            # print('pedge[2] is ' + str(pedge[2]) + '; threshold[a], threshold[a] are ' + str(threshold[a]) + ' ' + str(threshold[b]))
            if (pedge[2] <= threshold[a]) and (pedge[2] <= threshold[b]):
                u.join(a, b)
                a = u.find(a)
                threshold[a] = pedge[2] + get_threshold(u.size(a), c)
        if i == break_at_egde_i:
            print('break at edge index = ' + str(i) + ' total number edges = ' + str(num_edges))
            break
    return u
