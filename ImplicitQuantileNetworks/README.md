# Dopamine
NOTE: Original repo can be found [here](https://github.com/google/dopamine). This is a clone of the original repo with some minor fixes and some additional README documentation on requirements and Implicit Quantile Networks (IQN) (See end of this file!).
...
# ----ADDITIONS TO ORIGINAL README START HERE----
# Extra Requirements (Not Specified in original dopamine repo):

You may need to install SDL to install some of the python requirements. On Mac, you can do this by running
```
brew install sdl sdl_image sdl_mixer sdl_ttf portmidi
```
using homebrew first.

After installing SDL, please run the following commands, as the `requirements.txt` is not up to date. These requirements are not specified by Google, but are necessary (Tested on Mac using python 3.8.5).

```
python -m pip install -U pip
python -m pip install -r dopamine/requirements.txt
python -m pip install --upgrade gin-config
python -m pip install --upgrade jax
python -m pip install --upgrade jaxlib
python -m pip install --upgrade flax
```

# Implicit Quantile Networks (IQN)
The implementation of IQN can be found under the directory [`dopamine/agents/implicit_quantile`](https://github.com/google/dopamine/tree/master/dopamine/agents/implicit_quantile), with [`implicit_quantile_agent.py`](https://github.com/google/dopamine/blob/master/dopamine/agents/implicit_quantile/implicit_quantile_agent.py) containing the code specifying an IQN agent.

To run IQN on the Atari-57 environment with the same hyperparameters as the IQN paper, you can run the following command after following the original setup commands and the additional commands specified above:
```
python -um dopamine.discrete_domains.train \
--base_dir tmp/dopamine_runs \
--gin_files dopamine/agents/implicit_quantile/configs/implicit_quantile_icml.gin
```

This will run for 200 iterations using the configs as specified in `dopamine/agents/implicit_quantile/configs/implicit_quantile_icml.gin` (GPU required for somewhat reasonable runtime). Feel free to play around with the configs and check performance with different hyperparameter configurations!
